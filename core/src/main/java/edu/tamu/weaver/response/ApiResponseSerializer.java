...
public class ApiResponseSerializer extends JsonSerializer<ApiResponse> {
    ...
    @Override
    public void serialize(ApiResponse apiResponse, JsonGenerator jgen, SerializerProvider serializers) throws IOException {
        RequestAttributes attributes = RequestContextHolder.getRequestAttributes();
        if (attributes != null) {
            HttpServletRequest request = ((ServletRequestAttributes) attributes).getRequest();
            if (request != null && request.getHeader("x-wvr-unwrap") != null) {
