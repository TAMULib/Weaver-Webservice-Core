...
class MergeContext implements Map {
    ...
    /* package-private */ Object put(Object mergeEntity, Object managedEntity, boolean isOperatedOn) {
        ...
        if ( managedToMergeEntityXref.containsKey( mergeEntity ) ) {
            LOG.info("managedToMergeEntityXref: " + managedToMergeEntityXref + " contains mergeEntity: " + printEntity( mergeEntity ));
            if ( managedToMergeEntityXref.get( mergeEntity ) != mergeEntity ) {
                LOG.warn("managedToMergeEntityXref: " + managedToMergeEntityXref + " get mergeEntity: " + managedToMergeEntityXref.get( mergeEntity ) + " doesnt equal: " + printEntity( mergeEntity ));
                boolean dontdoit = true;
                if(dontdoit) {
                    LOG.warn("MergeContext#attempt to create managed -> managed mapping with different entities: "
                            + printEntity( mergeEntity ) + "; " + printEntity(
                            managedEntity ) + "Overriding exception! Allowing merge!");
                }
                else {
