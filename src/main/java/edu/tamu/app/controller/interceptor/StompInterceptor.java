	...
	public Message<?> preSend(Message<?> message, MessageChannel channel) {
		...
		if("SEND".equals(command.name())) {
	    	...
	    	if(error != null) {
	    		...
	    		((SimpMessagingTemplate) ApplicationContextProvider.appContext.getBean("brokerMessagingTemplate")).convertAndSend(accessor.getDestination().replace("ws", "queue") + "-user" + accessor.getSessionId(), new ApiResImpl("failure", error, new RequestId(requestId)));
	    		return null;
	    	}
