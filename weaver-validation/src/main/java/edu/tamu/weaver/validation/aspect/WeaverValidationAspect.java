...
public class WeaverValidationAspect {
        ...
        for (Annotation[] annotations : method.getParameterAnnotations()) {
            for (Annotation annotation : annotations) {
                if (annotation instanceof WeaverValidatedModel) {
                    validationResults = validateModel((ValidatingBaseEntity) arguments[index], method);
                    validated = true;
                    break;
                }
            }
    ...
    public <V extends ValidatingBaseEntity> ValidationResults validateModel(V model, Method method) {
        for (Annotation validationAnnotation : method.getAnnotations()) {
            if (validationAnnotation instanceof WeaverValidation) {
                for (WeaverValidation.Business businessAnnotation : ((WeaverValidation) validationAnnotation).business()) {
                    ((BaseModelValidator) ((ValidatingBaseEntity) model).getModelValidator()).addBusinessValidator(new BusinessValidator(businessAnnotation.value(), businessAnnotation.joins(), businessAnnotation.params(), businessAnnotation.path(), businessAnnotation.restrict()));
                }
            }
        }
        return ((ValidatingBaseEntity) model).validate((ValidatingBaseEntity) model);
    }
